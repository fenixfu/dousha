...
**2026-05 又补的一批**（QUA-129 一次性靠齐）：
- `enable_print_chinese: false`、`remove_space_between_han_num: false`、`remove_space_between_han_eng: false`、`strong_ddc: true` — 文本格式化/纠错平滑，对应官方 StartSession
- 设备指纹 `device_brand=google`、`device_model=Pixel 7 Pro`、`os=Android`、`os_version=16`、`app_version=1.1.2` — 与凭据注册时（`DoubaoCredentialStore.registerDevice`）用的同一份 `DoubaoConstants` 人设保持一致

**官方还设了但 dousha 仍没补的**（基本无关 / 拿不到）：
...
- `retry_server_code` — 客户端遇到这些 server code 时重连（值是 Bytedance 远程配置，本地拉不到，见 4.F）
- `context`、`s2a_*` — 识别上下文 / 语音指令；上下文功能已记 Linear QUA-133，后期可做
- `disable_user_words`、`join_user_experience_improve_program`、`dump_audio_path` — 词库/遥测/调试，跟掐流无关
...
### C. `frame_time_ms = 10`（✅ 已采纳，QUA-129）
...
官方用 10ms 帧。dousha 原来用 20ms，2026-05 已改成 10ms（`DoubaoConstants.frameDurationMs = 10`，OpusEncoder 全部从该常量派生，改一处即级联）。动机是服务器侧 VAD/部分结果节奏围绕官方 10ms 输入调过，更细的帧能更忠实地跟住短句和开头。实测无副作用、响应正常。
...
| StartSession config 关键字段 | 20+ | 17 | ✅ 行为相关的全补齐，剩下的是多链路/遥测/远程配置 |
| finish_audio 标记 | 是 | 是 | ✅ |
| finish_wait_timeout | 10s | 10s | ✅ |
| frame_time_ms | 10ms | 10ms | ✅ |
